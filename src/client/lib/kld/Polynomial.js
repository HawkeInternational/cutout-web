...
Polynomial.prototype.eval = function (x) {
    if (!(typeof x === 'number') || x !== x) // IE11 has slow isNaN()
...
Polynomial.prototype.getRootsInInterval = function (min, max) {
    ...
    if (this.getDegree() <= 3) {
        roots = this.getRoots().sort(function (a, b) { return a - b; });
        for (i = 0; i < roots.length;) {
        ...
        var df = function (x) { return deriv.eval(x); };
        var rootcalc = function (min, max) {
            var minValue = poly.eval(min);
            var maxValue = poly.eval(max);
            var result;
            ...
            if (Math.abs(minValue) <= Polynomial.TOLERANCE)
                ...
                return Polynomial.newton_secant_bisection((min + max) / 2, f, df, 32, min, max);
            }
            return result;
        }
        ...
        if (droots.length > 0) {
            // find root on [min, droots[0]]    
            root = rootcalc(min, droots[0]);
            if (root != null) roots.push(root);

            // find root on [droots[i],droots[i+1]] for 0 <= i <= count-2
            for (i = 0; i <= droots.length - 2; i++) {
                root = rootcalc(droots[i], droots[i + 1]);
                if (root != null) roots.push(root);
            }

            // find root on [droots[count-1],xmax]
            root = rootcalc(droots[droots.length - 1], max);
            if (root != null) roots.push(root);
        } else {
            // polynomial is monotone on [min,max], has at most one root
            root = rootcalc(min, max);
            if (root != null) roots.push(root);
        }
    }
...
*/
var ensure_array_length = function (a, len) {
    if (a.length >= len)
        return;
    for (var i = len - a.length; i > 0; i--)
        a.push(0);
}

var bounds_UpperReal_Fujiwara_a = [];
//ensure_array_length(bounds_UpperReal_Fujiwara_a, 10);
bounds_UpperReal_Fujiwara_a.length = 10;
var bounds_UpperReal_Fujiwara_b = [];
//ensure_array_length(bounds_UpperReal_Fujiwara_b, 10);
bounds_UpperReal_Fujiwara_b.length = 10;
Polynomial.prototype.bounds_UpperReal_Fujiwara = function () {
    ...
    var i, v;
    if (an != 1) {
        a = bounds_UpperReal_Fujiwara_a;
        ensure_array_length(a, n + 1);
        for (i = 0; i <= n; i++)
            a[i] = this.coefs[i] / an;
    }

    var b = bounds_UpperReal_Fujiwara_b;
    ensure_array_length(b, n + 1);
    for (i = 0; i <= n; i++) {
        v = a[i];
        b[i] = (i < n) ? Math.pow(Math.abs((i == 0) ? v / 2 : v), 1 / (n - i)) : v;
    }
    ...
    var max_nearmax_pos = { max: 0, nearmax: 0 };
    for (i = 0; i <= n; i++) {
        max_nearmax_pos = find2Max(max_nearmax_pos, b[i], i);
    }

    coefSelectionFunc = function (i) { return i < n && ((n % 2 == i % 2) ? a[i] < 0 : a[i] > 0); };
    var max_nearmax_neg = { max: 0, nearmax: 0 };
    for (i = 0; i <= n; i++) {
        max_nearmax_neg = find2Max(max_nearmax_neg, b[i], i);
    }
